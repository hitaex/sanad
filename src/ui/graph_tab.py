...
class GraphTab(QWidget):
    ...
    def create_graph_control_panel(self):
        ...
        self.export_png_btn.clicked.connect(self.export_pdf)
    ...
    def create_graph_toolbar(self):
        ...
        # Zoom controls
        toolbar_layout.addWidget(QLabel("التكبير:"))

        zoom_in_btn = StyledPushButton("➕")
        zoom_in_btn.setObjectName("toolbarButton")
        zoom_in_btn.setToolTip("Ctrl++")
        zoom_in_btn.clicked.connect(self.zoom_in)
        toolbar_layout.addWidget(zoom_in_btn)

        zoom_out_btn = StyledPushButton("➖")
        zoom_out_btn.setObjectName("toolbarButton")
        zoom_out_btn.setToolTip("Ctrl+-")
        zoom_out_btn.clicked.connect(self.zoom_out)
        toolbar_layout.addWidget(zoom_out_btn)

        zoom_reset_btn = StyledPushButton("🔄")
        zoom_reset_btn.setObjectName("toolbarButton")
        zoom_reset_btn.setToolTip("Ctrl+0")
        zoom_reset_btn.clicked.connect(self.zoom_reset)
        toolbar_layout.addWidget(zoom_reset_btn)

        zoom_fit_btn = StyledPushButton("⛶")
        zoom_fit_btn.setObjectName("toolbarButton")
        zoom_fit_btn.setToolTip("احتواء")
        ...
        toolbar_layout.addWidget(QLabel("تنسيق النص:"))
        ...
        self.font_size_combo.addItems([str(s) for s in [8, 9, 10, 11, 12, 14, 16, 18, 20, 24, 28, 32]])
        self.font_size_combo.setCurrentText("12")
        self.font_size_combo.currentTextChanged.connect(self.change_font_size)
        toolbar_layout.addWidget(self.font_size_combo)

        self.text_color_btn = StyledPushButton("🎨")
        self.text_color_btn.setFixedWidth(35)
    ...
    def toggle_bold(self):
        ...
        if text_item:
            ...
            # For NarratorNode and TextBox, we should notify the update
            if isinstance(text_item.parentItem(), (NarratorNode, TextBox)):
                text_item.parentItem().update_node_size() if hasattr(text_item.parentItem(), 'update_node_size') else None
    ...
    def add_selected_narrator(self):
        """Add selected narrator to chain or as child of pending parent instantly."""
        if not hasattr(self, 'narrator_list'):
            return

        # Safety measure: if multiple nodes are selected, don't allow adding a narrator
        ...
        # Clone the narrator data to avoid circular references
        import copy
        original_narrator = current_item.data(Qt.ItemDataRole.UserRole)
        narrator = copy.copy(original_narrator)
        
        # Ensure it has a unique ID if it's being added multiple times to the same chain
        if hasattr(narrator, 'children'):
            narrator.children = []
            
        method = self.method_combo.currentText()

        # Disable updates during addition and render
        self.setUpdatesEnabled(False)
        try:
            if self.pending_parent_id is not None:
                # Add as child to pending parent
                if self._add_narrator_to_parent(self.pending_parent_id, narrator, method):
                    self.main_window.status_label.setText(f"تمت إضافة: {narrator.name if hasattr(narrator, 'name') else narrator.get('name')} تابعاً")
                else:
                    self.main_window.status_label.setText(f"فشل إضافة التابع")
                # Do NOT clear pending_parent_id here as requested: "whenever i add it falls under it"
            else:
                # Simple linear addition
                self.chain_items.append((narrator, method))
                self.main_window.status_label.setText(f"تمت إضافة: {narrator.name if hasattr(narrator, 'name') else narrator.get('name')}")

            self.update_chain_list()
            # In Hierarchical mode, draw_chain needs to know where to find nodes.
            # In the current implementation, draw_chain/draw_vertical_tree uses recursive traversal of chain_items.
            self.draw_chain()
            
            # Re-apply highlight if we are in pending mode
            if self.pending_parent_id is not None:
    ...
    def _live_draw_chain(self):
        """Redraw graph after chain change instantly."""
    ...
    def update_chain_list(self):
        """Update the chain list widget with safeguard against recursion."""
    ...
    def on_chain_item_clicked(self, item):
        ...
        if item_type == 'NARRATOR':
            narrator_id = args[0]
            # When clicking a narrator in the list, we show its details
            self.show_narrator_details(narrator_id)
            
            # Select the node in the view if it exists
    ...
    def move_narrator_up(self):
        """Move selected item up instantly."""
        if not hasattr(self, 'chain_list'):
            return
    ...
    def move_narrator_down(self):
        """Move selected item down instantly."""
        if not hasattr(self, 'chain_list'):
            return
        current_row = self.chain_list.currentRow()
    ...
    def remove_narrator(self):
        ...
        # Check if any text boxes are selected first
        selected_items = self.chain_view.scene.selectedItems()
        ...
        if not current_item:
            # If no item is selected in the list, check if any node is selected on the canvas
    ...
    def remove_narrator_by_id(self, narrator_id):
        """Recursively remove narrator and its children from the chain instantly."""
        ...
        try:
            ...
            if remove_recursive(self.chain_items, narrator_id):
                # Reset pending parent if it was deleted
                ...
                # Re-apply highlight if we are in pending mode and the parent still exists
                if self.pending_parent_id is not None:
                    for n in self.chain_nodes:
                        if n.narrator_id == self.pending_parent_id:
                            n.setPen(QPen(QColor(0, 120, 215), 4, Qt.PenStyle.DashLine))
                ...
                self.main_window.status_label.setText(f"تم حذف الراوي")
        finally:
            self.setUpdatesEnabled(True)

    def clear_chain(self):
        """Clear the entire chain instantly."""
        ...
        if reply == QMessageBox.StandardButton.Yes:
            self.setUpdatesEnabled(False)
            try:
                # Recursively clear children to prevent memory leaks if any
    ...
    def draw_chain(self):
        """Draw the chain visualization with branch support."""
        # Clear scene
        if not hasattr(self, 'chain_view'):
            return

        # Set updates disabled to prevent flickering
        ...
        try:
            ...
            if not self.chain_items:
                # If empty, just return after clearing
                return
        ...
        self.main_window.status_label.setText(f"✅ تم رسم السند")

    def draw_vertical_tree(self):
        """Draw vertical tree layout with branch support."""
    ...
    def add_narrator_after_node(self, parent_narrator_id, node):
        """Designate parent node for next addition instantly."""
        # Toggle highlight: if already designated, deselect it
        if self.pending_parent_id == parent_narrator_id:
            ...
            self.main_window.status_label.setText("بانتظار اختيار راوٍ (من اليمين) أو صندوق فارغ (من اليسار) لإضافته تابعاً...")

        self.setUpdatesEnabled(False)
        try:
            # Visually indicate which node is the pending parent
            for n in self.chain_nodes:
                n_id = n.narrator_id
                if self.pending_parent_id is not None and n_id == self.pending_parent_id:
                    n.setPen(QPen(QColor(0, 120, 215), 4, Qt.PenStyle.DashLine))
                    # Make it darker/highlighted as requested
                    n.setBrush(QBrush(n.color.darker(110)))
                else:
    ...
    def add_branch_from_node(self, narrator_id, node):
        """DEPRECATED: Now handled by add_narrator_after_node (multiple children)."""
        self.add_narrator_after_node(narrator_id, node)

    def add_sub_branch_to_branch(self, branch_index):
        """Add a new sub-branch to an existing branch point instantly."""
        ...
        try:
            ...
            # Ensure it's expanded to see the new branch
    ...
    def node_is_branch(self, narrator_id):
        """Check if a node is actually a branch point."""
    ...
    def add_blank_box_to_chain(self):
        """Add a blank, editable text box to the chain or as child instantly."""
        # Safety measure: if multiple nodes are selected, don't allow adding a narrator
        selected_nodes = [item for item in self.chain_view.scene.selectedItems() if isinstance(item, NarratorNode)]
        if len(selected_nodes) > 1:
        ...
        try:
            if self.pending_parent_id is not None:
                ...
                # Do NOT clear pending_parent_id here
    ...
    def add_text_box_to_canvas(self):
        """Add text box to canvas at center of visible area."""
        if not hasattr(self, 'chain_view'):
            return
    ...
    def show_narrator_details(self, narrator_id):
        """Show narrator details in graphing tab's right panel."""
        ...
        if hasattr(self, 'graph_basic_text'):
            self.graph_basic_text.setHtml(basic_html if basic_info else "<div style='text-align: center; color: #808080; padding: 20px;'>لا توجد معلومات</div>")
    ...
    def on_canvas_clicked(self):
        ...
        try:
            ...
            # Reset pending parent visual if clicking elsewhere
            if self.pending_parent_id is not None:
                self.pending_parent_id = None
                for n in self.chain_nodes:
                    n.setPen(QPen(n.border_color, 2))
    ...
    def export_image(self, format='png'):
        ...
        if not hasattr(self, 'chain_view') or not hasattr(self, 'chain_nodes') or not self.chain_nodes:
        ...
        self.main_window.status_label.setText(f"✅ تم حفظ الصورة")
    ...
    def export_pdf(self):
        ...
        self.main_window.status_label.setText(f"✅ تم حفظ PDF")
    ...
    def save_chain(self):
        ...
        self.export_controller.save_chain(self.chain_items, data, self.main_window.narrators_dict, self.chain_view.scene)
        self.main_window.status_label.setText(f"✅ تم حفظ السند")
    ...
    def copy_to_clipboard(self):
        ...
        # Simplified: copy the first selected narrator for now
    ...
    def paste_from_clipboard(self):
        """Paste narrator from clipboard as child of selected or at root."""
        ...
        # Ensure it has a unique ID if it's a custom/blank one
            ...
            try:
                ...
                if 'text_boxes' in data:
                    for tb_data in data['text_boxes']:
                        ...
                        # Apply visual styles
                        if 'width' in tb_data and 'height' in tb_data:
                            text_box.setRect(0, 0, tb_data['width'], tb_data['height'])
                        if 'color' in tb_data:
                        ...
                        text_box.text_item.setPos(
                            (text_box.rect().width() - text_rect.width()) / 2,
                            (text_box.rect().height() - text_rect.height()) / 2
                        )
